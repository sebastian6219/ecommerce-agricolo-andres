
import React, { useContext, useEffect, useState } from "react";
import { CartContext } from "../../../../context/CartContext";
import CartItem from "./CartItem";
import "./cart.css";
import { Link } from "react-router-dom";


const Cart = () => {
    const { cart, vaciarCarrito } = useContext(CartContext);
    const [precioTotal, setPrecioTotal] = useState(0);



    useEffect(() => {

        const total = cart.reduce((acc, item) => acc + (item.price * item.cantidad), 0);
        setPrecioTotal(total);

    }, [cart]);

    return (
        <div>
            {cart.length > 0 ? (
                <>
                    {cart.map(e => (
                        <CartItem key={e.id} producto={e} />
                    ))}
                    <div className="price-and-buttons">
                        <p className="price-total">Precio Total: ${precioTotal}</p>
                        <div className="buttons">
                            <button className="btn-clear-cart" onClick={vaciarCarrito}>
                                Vaciar Carrito
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-trash" viewBox="0 0 16 16">
                                    <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/>
                                    <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
                                </svg>

                            </button>
                            <Link to={"/cart/checkout"}>
                                <button className="btn-checkout">Terminar Compra</button>
                            </Link>
                        </div>
                    </div>
                </>
            ) : (
                <div className="div-container">
                    <div className="date-contain">
                        <div>

                        </div>
                        <div>
                            <h2>Todavía no tienes nada en tu carrito</h2>
                            <h3>Ven y mira todos nuestros productos</h3>
                            <Link to={"/"}>
                                <button className="btn-blue">Descubrir Productos</button>
                            </Link>
                        </div>
                    </div>
                </div>
            )}
        </div>
    );
};

export default Cart